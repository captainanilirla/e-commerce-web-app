...
from shoping.models import Product,Buy
from shoping.forms import CartForm
from shoping.myapp import *
...
def details(request,product_id,slug):
    ...
    if request.method == 'POST':
        ...
        if f.is_valid(): 
            ...
            add_to_cart(request)
            return redirect('shoping:cart_view')
...
def checkout(request):
    ...
    paypal_dict={
            ...
            "return":request.build_absolute_uri(reverse("shoping:return_view")),
            "cancel":request.build_absolute_uri(reverse("shoping:cancel_view")),
            "custom":"premium_plan",
    }
